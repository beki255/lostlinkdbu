    ...
    <div className="page-container max-w-2xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500">{user?.email} &middot; <span className="capitalize">{user?.role}</span></p>
        </div>
      </div>
      ...
      <div className="card">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiBook className="w-4 h-4 inline mr-1" /> Department
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <FiMail className="w-4 h-4 inline mr-1" /> Student ID
            </label>
            <input type="text" value={form.studentId} onChange={(e) => setForm({ ...form, studentId: e.target.value })}
              className="input-field" placeholder="e.g., DBU/2020/001" />
          </div>
